<template>
    <div class="w-full pt-9">
                            ...
                            <div class="flex justify-end mt-9">
                                <template v-for="image in item.imageList">
                                    <img @click="handleTapImage(image)"
                                        class="w-3/4 bg-gray-50 object-contain hover:cursor-pointer" :src="image" />
                                </template>
                            </div>
                        </div>
                    </div>
        ...
        <template v-if="imageListCount == 2">
            <template v-if="hasTitleImageImage">
                <div class="flex">
                    <div class="w-2/5">
                        <template v-for="mainImage in item.mainImageList">
                            <img @click="handleTapImage(mainImage)" class="w-full object-contain hover:cursor-pointer"
                                :src="mainImage" alt="">
                        </template>
                    </div>
                        ...
                        <div class="w-full flex">
                            ...
                            <div class="w-1/3 flex justify-end">
                                <img @click="handleTapImage(mainImage)"
                                    class="w-64 mb-4 object-contain hover:cursor-pointer" :src="item.titleImage" alt="">
                            </div>
                        </div>
                        <div class="mt-9 ml-9">
                            <div class="">{{ item.description }}</div>
                            <div class="absolute bottom-0 right-0">
                                <div class="flex justify-end mt-9">
                                    <template v-for="image in item.imageList">
                                        <img @click="handleTapImage(image)"
                                            class="w-64 ml-3 object-contain hover:cursor-pointer" :src="image" />
                                    </template>
                                </div>
                            </div>
                        </div>
